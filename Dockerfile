# Create Builder & Build
FROM golang:1.11.4-alpine3.8 as builder

RUN apk update && apk add --no-cache git && apk add ca-certificates
RUN adduser -D -g '' appuser

COPY . $GOPATH/src/github.com/stefancrain/darksky-exporter
WORKDIR $GOPATH/src/github.com/stefancrain/darksky-exporter

# Run Test
RUN go get golang.org/x/tools/cmd/cover
RUN go get github.com/go-playground/overalls
RUN overalls -covermode=atomic -project=github.com/stefancrain/darksky-exporter -- -race -v
RUN mv overalls.coverprofile darksky-exporter.cover
RUN go tool cover -func=darksky-exporter.cover

# Get Dependencies
RUN go get -d -v

# Build Binary in builder
RUN CGO_ENABLED=0 GOOS="linux" GOARCH="arm64" go build -a -installsuffix cgo -ldflags="-w -s" -o /app
# RUN CGO_ENABLED=0 GOOS="linux" GOARCH="amd64" go build -a -installsuffix cgo -ldflags="-w -s" -o /app

# Create Container
FROM scratch

MAINTAINER Stefan Crain <stefancrain@gmail.com>

COPY --from=builder /etc/ssl/certs/ca-certificates.crt /etc/ssl/certs/
COPY --from=builder /etc/passwd /etc/passwd
COPY --from=builder /app /app

USER appuser
ENTRYPOINT ["/app"]
